// Updated implementation of AIService without Firebase Functions dependency
// Now using standard HTTP client with our custom Render.com backend
...
class AIService {
  ...
  Future<void> _getStreamingResponse(
    ...
    try {
      ...
      if (_hasInitializationError) {
        throw Exception("AI service not available - using fallback");
      }
      ...
      try {
        // Use HTTP to call our custom API instead of Firebase Functions
        final response = await http
            .post(
              Uri.parse('${FoodAnalyzerApi.baseUrl}/ai-coach'),
              headers: {'Content-Type': 'application/json'},
              body: jsonEncode({'messages': safeMessages}),
            )
        ...
        print('AI Service: HTTP call error: $callError');
  ...
  Future<void> _fallbackToNonStreaming(List<Map<String, String>> safeMessages,
    ...
    try {
      ...
      // Use HTTP to call our custom API
      final response = await http
          .post(
            Uri.parse('${FoodAnalyzerApi.baseUrl}/ai-coach'),
            headers: {'Content-Type': 'application/json'},
...
''';
  ...
  /// Function to analyze a food image
  Future<Map<String, dynamic>> analyzeFoodImage(Uint8List imageBytes) async {
    try {
      // We now use the FoodAnalyzerApi class directly
